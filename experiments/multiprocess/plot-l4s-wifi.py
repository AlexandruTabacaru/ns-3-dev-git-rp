...
f = open(resultsDir + "/" + "wifi-throughput.dat", "r")
wifi_time = []
wifi_samples_plot = []
wifi_samples_avg = []
for line in f:
...
cubic_samples_plot = []
cubic_samples_avg = []
if os.path.exists(resultsDir + "/" + "cubic-throughput.dat"):
    f = open(resultsDir + "/" + "cubic-throughput.dat", "r")
    for line in f:
        columns = line.split()
        t = float(columns[0])
        cubic_time.append(t)
...
if os.path.exists(resultsDir + "/" + "cubic-rtt.dat"):
    f = open(resultsDir + "/" + "cubic-rtt.dat", "r")
    for line in f:
        columns = line.split()
        t = float(columns[0])
...
if os.path.exists(resultsDir + "/" + "cubic-cwnd.dat"):
    f = open(resultsDir + "/" + "cubic-cwnd.dat", "r")
...
prague_samples_plot = []
prague_samples_avg = []
if os.path.exists(resultsDir + "/" + "prague-throughput.dat"):
    f = open(resultsDir + "/" + "prague-throughput.dat", "r")
    for line in f:
        columns = line.split()
        t = float(columns[0])
...
if os.path.exists(resultsDir + "/" + "prague-rtt.dat"):
    f = open(resultsDir + "/" + "prague-rtt.dat", "r")
    for line in f:
        columns = line.split()
        t = float(columns[0])
...
if os.path.exists(resultsDir + "/" + "prague-cwnd.dat"):
    f = open(resultsDir + "/" + "prague-cwnd.dat", "r")
    for line in f:
        columns = line.split()
        t = float(columns[0])
...
f = open(resultsDir + "/" + "wifi-queue-bytes.dat", "r")
bytes_time = []
bytes_samples_plot = []
bytes_samples_avg = []
for line in f:
...
f = open(resultsDir + "/" + "wifi-dualpi2-bytes.dat", "r")
dualpi2_time = []
...
ax1.set_title(
    "Access point sending rate (all flows, measured at MAC layer)", fontsize=8
)
...
if add_horizontal_line_for_averaging:
    if len(cubic_samples_avg):
        ...
        cubic_label = r"Avg {} Mbps (2s < t < {}s)".format(
            cubic_avg, end_of_averaging_period
        )
...
if add_horizontal_line_for_averaging:
    if len(prague_samples_avg):
        ...
        prague_label = r"Avg {} Mbps (2s < t < {}s)".format(
            prague_avg, end_of_averaging_period
        )
