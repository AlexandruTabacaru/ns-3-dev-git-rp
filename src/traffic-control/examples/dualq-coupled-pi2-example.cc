...
NS_LOG_COMPONENT_DEFINE ("DualQCoupledPi2Example");
...
main (int argc, char *argv[])
{
  ...
  bool printDualQCoupledPi2Stats = true;
  ...
  // DualQCoupledPi2 parameters
  NS_LOG_INFO ("Set DualQCoupledPi2 params");
  Config::SetDefault ("ns3::DualQCoupledPi2QueueDisc::QueueLimit", UintegerValue (50000));
  ...
  handle = tch.SetRootQueueDisc ("ns3::DualQCoupledPi2QueueDisc");
  tch.AddInternalQueues (handle, 2, "ns3::DropTailQueue", "MaxSize", StringValue ("50000B"));
  ...
  // only backbone link has DualQCoupledPi2 queue disc
  ...
  if (printDualQCoupledPi2Stats)
    {
      std::cout << "*** DualQCoupledPi2 stats from Node 2 queue ***" << std::endl;
