...
private:
  void Enqueue (Ptr<DualPi2QueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType);
  void EnqueueWithDelay (Ptr<DualPi2QueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType);
...
DualPi2QueueDiscTestCase::RunPiSquareTest (void)
{
  ...
  Simulator::Run ();
  uint32_t test2ClassicMark = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_CLASSIC_MARK);
  uint32_t test2L4SMark = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_L4S_MARK);
  ...
  NS_TEST_EXPECT_MSG_NE (st.GetNDroppedPackets (DualPi2QueueDisc::FORCED_DROP), 0, "There should be some forced drops");
  ...
  Simulator::Run ();
  st = queue->GetStats ();
  uint32_t test3ClassicDrop = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_CLASSIC_DROP);
  NS_TEST_EXPECT_MSG_EQ (test3ClassicDrop, 0, "There should be zero unforced classic drops since only L4S traffic is pumped ");
  uint32_t test3ClassicMark = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_CLASSIC_MARK);
  NS_TEST_EXPECT_MSG_EQ (test3ClassicMark, 0, "There should be zero unforced classic marks since only L4S traffic is pumped");
  uint32_t test3L4SMark = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_L4S_MARK);
  ...
  Simulator::Stop (Seconds (8.0));
  Simulator::Run ();
  uint32_t test4ClassicDrop = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_CLASSIC_DROP);
  NS_TEST_EXPECT_MSG_EQ (test4ClassicDrop, 0, "There should be zero unforced classic drops since packets are ECN capable ");
  uint32_t test4ClassicMark = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_CLASSIC_MARK);
  NS_TEST_EXPECT_MSG_NE (test4ClassicMark, 0, "There should be some unforced classic marks");
  uint32_t test4L4SMark = queue->GetStats ().GetNMarkedPackets (DualPi2QueueDisc::UNFORCED_L4S_MARK);
  NS_TEST_EXPECT_MSG_EQ (test4L4SMark, 0, "There should be zero L4S marks since only Classic traffic is pumped");
}

void
DualPi2QueueDiscTestCase::Enqueue (Ptr<DualPi2QueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType)
{
  Address dest;
  for (uint32_t i = 0; i < nPkt; i++)
    {
      if (trafficType.Get () == "L4S")
        {
          Ptr<DualQueueL4SQueueDiscTestItem> item = Create<DualQueueL4SQueueDiscTestItem> (Create<Packet> (size), dest, 0);
          queue->Enqueue (item);
        }
...
void
DualPi2QueueDiscTestCase::EnqueueWithDelay (Ptr<DualPi2QueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType)
{
  Address dest;
  double delay = 0.01;  // enqueue packets with delay
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds (i * delay)), &DualPi2QueueDiscTestCase::Enqueue, this, queue, size, 1, trafficType);
    }
}
...
DualPi2QueueDiscTestCase::DequeueWithDelay (Ptr<DualPi2QueueDisc> queue, double delay, uint32_t nPkt)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &DualPi2QueueDiscTestCase::Dequeue, this, queue, 1);
    }
}
