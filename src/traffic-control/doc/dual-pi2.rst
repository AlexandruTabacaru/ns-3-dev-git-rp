...
Dual PI Squared [Schepper17]_ is a Dual Queue Coupled AQM algorithm
based on the PI2 algorithm [Schepper16]_ for the Classic AQM. PI2 is an
improved variant of the PIE AQM [Pan16]_.  In |ns3|, it is named
...
algorithm:
  ...
  * ``DualPi2QueueDisc::DoEnqueue ()``: This routine checks whether
  ...
  * ``DualPi2QueueDisc::CalculateP ()``: This routine is called at
  ...
  * ``DualPi2QueueDisc::DoDequeue ()``: This routine schedules
...
The example for DualPI2 is `dual-pi2-example.cc` located in
...
DualPI2 model is tested using :cpp:class:`DualPi2QueueDiscTestSuite`
class defined in `src/traffic-control/test/dual-pi2-queue-disc-test-suite.cc`.
...
::
  ...
  $ NS_LOG="DualPi2QueueDisc" ./waf --run "test-runner --suite=dual-pi2-queue-disc"
