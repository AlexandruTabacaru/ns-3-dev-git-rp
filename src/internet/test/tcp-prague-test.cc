...
class TcpPragueSlowStartTest : public TestCase
{
  public:
     ...
     */
    TcpPragueSlowStartTest(uint32_t cWnd,
...
TcpPragueSlowStartTest::TcpPragueSlowStartTest(uint32_t cWnd,
                                               uint32_t segmentSize,
                                               uint32_t ssThresh,
                                               uint32_t segmentsAcked,
                                               SequenceNumber32 highTxMark,
...
TcpPragueSlowStartTest::ExecuteTest()
{
    ...
    NS_TEST_ASSERT_MSG_LT(m_state->m_cWnd.Get(),
                          m_state->m_ssThresh.Get(),
                          "cWnd should be less than ssThresh in Slow Start test");
...
class TcpPragueCongestionAvoidanceTest : public TestCase
{
  public:
     ...
     * \param name Name of the test
     */
    TcpPragueCongestionAvoidanceTest(uint32_t cWnd,
                                     uint32_t segmentSize,
                                     uint32_t ssThresh,
                                     uint32_t segmentsAcked,
                                     SequenceNumber32 highTxMark,
...
TcpPragueCongestionAvoidanceTest::TcpPragueCongestionAvoidanceTest(uint32_t cWnd,
                                                                   uint32_t segmentSize,
                                                                   uint32_t ssThresh,
                                                                   uint32_t segmentsAcked,
                                                                   SequenceNumber32 highTxMark,
...
TcpPragueCongestionAvoidanceTest::ExecuteTest()
{
    ...
    NS_TEST_ASSERT_MSG_LT(m_state->m_ssThresh.Get(),
                          m_state->m_cWnd.Get(),
                          "cWnd should be more than ssThresh in Congestion Avoidance test");
...
class TcpPragueRttIndependenceTest : public TestCase
{
  public:
     ...
     * \param name Name of the test
     */
    TcpPragueRttIndependenceTest(uint32_t cWnd,
                                 uint32_t segmentSize,
                                 uint32_t ssThresh,
                                 uint32_t segmentsAcked,
                                 SequenceNumber32 highTxMark,
                                 SequenceNumber32 lastAckedSeq,
                                 Time rtt,
                                 TcpPrague::RttScalingMode_t mode,
                                 const std::string& name);
...
TcpPragueRttIndependenceTest::TcpPragueRttIndependenceTest(uint32_t cWnd,
                                                           uint32_t segmentSize,
                                                           uint32_t ssThresh,
                                                           uint32_t segmentsAcked,
                                                           SequenceNumber32 highTxMark,
...
TcpPragueRttIndependenceTest::ExecuteTest()
{
    ...
    NS_TEST_ASSERT_MSG_LT(m_state->m_ssThresh.Get(),
                          m_state->m_cWnd.Get(),
                          "RTT independence should be tested in Congestion Avoidance");
    ...
    congPrague->SetRttTransitionDelay(
        0); // enforce rtt independence immediately in congestion avoidance
    congPrague->Init(m_state);
    ...
    NS_TEST_ASSERT_MSG_EQ(m_cWndCnt,
                          cWndCnt,
...
TcpPragueRttIndependenceTest::GetAdditiveFactor()
{
    ...
    if (m_mode == TcpPrague::RTT_CONTROL_RATE || m_mode == TcpPrague::RTT_CONTROL_ADDITIVE)
    {
        ...
        return 1.0 * m_rtt.GetSeconds() * m_rtt.GetSeconds() /
...
class TcpPragueTestSuite : public TestSuite
{
  public:
    TcpPragueTestSuite()
        : TestSuite("tcp-prague-test", UNIT)
    {
        // slow start test, cWnd < ssThresh
        // to ensure that Prague behaves like LinuxReno, it is better to set segmentsAcked to a
        // value greater than 1
        AddTestCase(
            new TcpPragueSlowStartTest(2 * 1446,
                                       1446,
                                       4 * 1446,
                                       2,
                                       ...
                                       MilliSeconds(10),
                                       "TcpPrague behaves like TcpLinuxReno during Slow Start"),
            TestCase::QUICK);

        // congestion avoidance test, cWnd >= ssThresh
        AddTestCase(new TcpPragueCongestionAvoidanceTest(
                        5 * 1446,
                        1446,
                        3 * 1446,
                        2,
                        ...
                        SequenceNumber32(3216),
                        MilliSeconds(10),
                        "TcpPrague behaves like TcpLinuxReno during Congestion Avoidance"),
                    TestCase::QUICK);

        // rtt independence is handled in congestion avoidance, cWnd >= ssThresh
        AddTestCase(new TcpPragueRttIndependenceTest(
                        5 * 1446,
                        1446,
                        3 * 1446,
                        2,
                        ...
                        SequenceNumber32(3216),
                        MilliSeconds(10),
                        TcpPrague::RTT_CONTROL_NONE,
                        "TcpPrague with the RTT_CONTROL_NONE scaling heuristic"),
                    TestCase::QUICK);
        ...
        AddTestCase(new TcpPragueRttIndependenceTest(
                        ...
                        "TcpPrague with the RTT_CONTROL_RATE scaling heuristic"),
                    TestCase::QUICK);
        ...
        AddTestCase(new TcpPragueRttIndependenceTest(
                        ...
                        "TcpPrague with the RTT_CONTROL_SCALABLE scaling heuristic"),
                    TestCase::QUICK);
        ...
        AddTestCase(new TcpPragueRttIndependenceTest(
                        ...
                        "TcpPrague with the RTT_CONTROL_ADDITIVE scaling heuristic"),
                    TestCase::QUICK);
    }
};
