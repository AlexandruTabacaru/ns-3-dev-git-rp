...
{
    if (init_bw && m_bwLo == std::numeric_limits<u_int32_t>::max ())
        m_bwLo = bbr_max_bw();
...
TcpBbr3::bbr_is_inflight_too_high(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    //     }
    // }
    if (rs.m_bytesLoss > 0u && tcb->m_bytesInFlight > 0u)
    {
...
{
    NS_LOG_FUNCTION (this);
    if (m_inflightHi == std::numeric_limits<u_int32_t>::max ())
...
TcpBbr3::bbr_bound_cwnd_for_inflight_model(Ptr<TcpSocketState> tcb)
{
    ...
    if (!m_isInitialized)
        return;
    uint32_t cap = std::numeric_limits<u_int32_t>::max();   
...
TcpBbr3::bbr_start_bw_probe_down()
{
    ...
    m_bwProbeUpCount = std::numeric_limits<u_int32_t>::max ();
    bbr_pick_probe_wait();
    m_cycleStamp = Simulator::Now();
...
{
    m_cycleStamp = Simulator::Now();
    if (m_inflightLo != std::numeric_limits<u_int32_t>::max ())
...
{
    m_bwLo = std::numeric_limits<u_int32_t>::max(); 
    m_inflightLo = std::numeric_limits<u_int32_t>::max();
}
...
TcpBbr3::CongestionStateSet(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCongState_t newState)
{
    ...
    if (newState == TcpSocketState::CA_OPEN && !m_isInitialized) {
        ...
        m_roundStart = true;
        bbr_reset_full_bw();
        if (bbr_is_probing_bandwidth(tcb) && m_inflightLo == std::numeric_limits<u_int32_t>::max ())
        {
