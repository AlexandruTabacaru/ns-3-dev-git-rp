...
const double TcpBbr2::PACING_GAIN_CYCLE [] = {5.0 / 4, 3.0 / 4, 1, 1, 1, 1, 1, 1};
const double TcpBbr2::PACING_GAIN_CYCLE_HSR [] = {3.0 / 2, 0.5, 3.0 /2, 0.5, 3.0 / 2, 0.5, 3.0 / 2, 0.5};
const double TcpBbr2::PACING_GAIN_CYCLE_DELAY [] = {1.11, 0.9, 1, 1, 1, 1, 1, 1};

TypeId
TcpBbr2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpBbr2")
    ...
    .AddAttribute ("HighGain",
                   "Value of high gain",
                   DoubleValue (2.89),
                   MakeDoubleAccessor (&TcpBbr2::m_highGain),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BwWindowLength",
                   "Length of bandwidth windowed filter",
                   UintegerValue (10),
                   MakeUintegerAccessor (&TcpBbr2::m_bandwidthWindowLength),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RttWindowLength",
                   "Length of bandwidth windowed filter",
                   TimeValue (Seconds (10)),
                   MakeTimeAccessor (&TcpBbr2::m_rtPropFilterLen),
                   MakeTimeChecker ())
    .AddAttribute ("ProbeRttDuration",
                   "Length of bandwidth windowed filter",
                   TimeValue (MilliSeconds (200)),
                   MakeTimeAccessor (&TcpBbr2::m_probeRttDuration),
                   MakeTimeChecker ())
    .AddAttribute ("BBRVariant",
                   "Variant of BBR to use",
                   EnumValue (TcpBbr2::BBR),
                   MakeEnumAccessor (&TcpBbr2::m_variant),
...
int64_t
TcpBbr2::AssignStreams (int64_t stream)
{
...
void
TcpBbr2::InitPacingRate (Ptr<TcpSocketState> tcb)
{
  ...
  Time rtt = tcb->m_lastRtt.Get() != Time::Max () ? tcb->m_lastRtt.Get() : MilliSeconds (1);
...
void
TcpBbr2::HandleRestartFromIdle (Ptr<TcpSocketState> tcb, const RateSample * rs)
{
...
void
TcpBbr2::SetPacingRate (Ptr<TcpSocketState> tcb, double gain)
{
...
uint32_t
TcpBbr2::InFlight (Ptr<TcpSocketState> tcb, double gain)
{
  NS_LOG_FUNCTION (this << tcb << gain);
...
TcpBbr2::AdvanceCyclePhase ()
{
  ...
  else if (m_variant == TcpBbr2::BBR_DELAY)
    {
...
bool
TcpBbr2::IsNextCyclePhase (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  ...
  if (m_pacingGain == 1)
    ...
    {
      return isFullLength && (rs->m_packetLoss > 0 || tcb->m_bytesInFlight.Get() >= InFlight (tcb, m_pacingGain));
    }
  else
    {
      if (m_variant == BbrVar::BBR)
        ...
        {              
          return tcb->m_bytesInFlight.Get() <= InFlight (tcb, 1);
        }
      else
        {
          return isFullLength || tcb->m_bytesInFlight.Get() <= InFlight (tcb, 1);
        } 
    }
}

void
TcpBbr2::CheckCyclePhase (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::CheckFullPipe (const struct RateSample * rs)
{
...
void
TcpBbr2::EnterProbeBW (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this);
...
void
TcpBbr2::CheckDrain (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  ...
  if (m_state == BbrMode_t::BBR_DRAIN && tcb->m_bytesInFlight <= InFlight (tcb, 1))
    {
      ...
      EnterProbeBW (tcb);
      // Note: m_lastSentSeq not available, using next sequence number
      m_sentPacketNum = tcb->m_nextTxSequence.Get().GetValue();
    }
}

void
TcpBbr2::UpdateRTprop (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  ...
  if (tcb->m_lastRtt >= Seconds (0) && (tcb->m_lastRtt <= m_rtProp || m_rtPropExpired))
    {
      if (m_variant == BbrVar::BBR_HSR)
        {
          // Note: m_rttVar not available in current ns-3, using simplified approach
          m_rtProp = tcb->m_lastRtt.Get();
        }
              else 
        {
          m_rtProp = tcb->m_lastRtt.Get();
...
void
TcpBbr2::SaveCwnd (Ptr<const TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::RestoreCwnd (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::ExitProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this);
  // Reset lower bounds
...
void
TcpBbr2::HandleProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  // Note: m_appLimited not available in current ns-3, using alternative approach
  ...
  || (tcb->m_bytesInFlight <= InFlight (tcb, 0.75) && m_variant == BbrVar::BBR_DELAY)))
    {
      ...
      // Note: m_delivered not available, using sequence tracking
      m_nextRoundDelivered = tcb->m_nextTxSequence.Get().GetValue();
    }
...
void
TcpBbr2::CheckProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::SetSendQuantum (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::UpdateTargetCwnd (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::ModulateCwndForRecovery (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::ModulateCwndForProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  if (m_state == BbrMode_t::BBR_PROBE_RTT)
    {
...
void
TcpBbr2::SetCwnd (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  ...
  if (!m_packetConservation)
    {
      ...
      else if (tcb->m_cWnd < m_targetCWnd || tcb->m_bytesInFlight.Get() < tcb->m_initialCWnd * tcb->m_segmentSize)
        {
...
void
TcpBbr2::UpdateRound (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  // Note: m_txItemDelivered not available, using alternative round tracking
  if (rs->m_delivered >= m_nextRoundDelivered)
    {
      // Note: m_delivered not available, using sequence tracking
...
void
TcpBbr2::UpdateBtlBw (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::UpdateModelAndState (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  UpdateBtlBw (tcb, rs);
  if (m_variant == BbrVar::BBR_DELAY)
...
void
TcpBbr2::UpdateControlParameters (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
std::string
TcpBbr2::WhichState (BbrMode_t mode) const
{
  switch (mode)
    {
    case 0:
...
void
TcpBbr2::SetBbrState (BbrMode_t mode)
{
...
std::string
TcpBbr2::GetName () const
{
  return "TcpBbr2";
}

bool
TcpBbr2::HasCongControl () const
{
  NS_LOG_FUNCTION (this);
  return true;
}
...
void
TcpBbr2::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                        const Time& rtt)
{
  NS_LOG_FUNCTION (this << tcb << segmentsAcked << rtt);
  // Note: m_rs not available, creating dummy RateSample
  RateSample rs = {};
  rs.m_delivered = segmentsAcked;
  rs.m_deliveryRate = DataRate(tcb->m_cWnd.Get() * tcb->m_segmentSize * 8 / rtt.GetSeconds());
  CongControl (tcb, &rs);
}

void
TcpBbr2::CongControl (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::CongestionStateSet (Ptr<TcpSocketState> tcb,
...
void
TcpBbr2::CwndEvent (Ptr<TcpSocketState> tcb,
                   const TcpSocketState::TcpCAEvent_t event)
{
  ...
  if (event == TcpSocketState::CA_EVENT_COMPLETE_CWR)
    ...
    {
      // Note: Simplified ECN handling due to missing members in current ns-3
      m_deliveredEce += 1; // Count ECN events
      m_isEce = true;
    }
}

uint32_t
TcpBbr2::GetBbrVariant ()
{
  NS_LOG_FUNCTION (this);
  return m_variant;
}
...
void 
TcpBbr2::SetBbrVariant (BbrVar variant)
{
...
void
TcpBbr2::DrainToTargetCycling (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  if (m_state != BbrMode_t::BBR_PROBE_BW)
    {
  ...
  if (m_pacingGain < PACING_GAIN_CYCLE [BbrBwPhase::BBR_BW_PROBE_CRUISE])
    {
      if (tcb->m_bytesInFlight.Get() <= InFlight (tcb, 1))
        {
          SetCycleIndex (BbrBwPhase::BBR_BW_PROBE_CRUISE);
        }
      return;
    }
  if ((Simulator::Now () - m_cycleStamp) > m_rtProp && ((tcb->m_bytesInFlight.Get() <= InFlight (tcb, m_pacingGain)) || 
...
void
TcpBbr2::SetCycleIndex (BbrBwPhase index)
{
...
void
TcpBbr2::CheckCongestionDelay (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  if (m_state == BbrMode_t::BBR_PROBE_BW && tcb->m_lastAckedSeq.GetValue() > m_sentPacketNum)
    {
      if (m_baseRtt != Time::Max ())
        {
          int64_t m_old_srtt = m_srtt.GetMilliSeconds ();
          int64_t m_new_rtt = tcb->m_lastRtt.Get().GetMilliSeconds ();
...
void
TcpBbr2::UpdateCongestionSignals (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  m_lossRoundStart = 0;
  ...
  // Note: m_delivered not available, using alternative tracking
  m_lossRoundDelivered = rs->m_delivered;
  m_lossRoundStart = 1;
...
void
TcpBbr2::ResetCongestionSignals ()
{
  NS_LOG_FUNCTION (this);
  m_lossInRound = 0;
  m_ecnInRound = 0;
...
void
TcpBbr2::AdaptLowerBounds (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
bool
TcpBbr2::AdaptUpperBounds (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::CheckExcessiveLossStartup (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
bool
TcpBbr2::IsInflightTooHigh (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::HandleInflightTooHigh (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbr2::HandleInflightTooHighEcn ()
{
...
void
TcpBbr2::ProbeInflightHighUpward (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);

  // Don't grow inflight_hi if it is not fully used
  if (!(tcb->m_bytesInFlight.Get() >= m_priorCwnd) || tcb->m_initialCWnd < m_inflightHi)
    {
      m_bwProbeUpAcks = 0;
      return;
    }
...
void
TcpBbr2::RaiseInflightHiSlope (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
bool
TcpBbr2::IsTimeToProbe (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  bool isFullLength = (Simulator::Now () - m_cycleStamp) > m_rtProp;
  ...
  if (m_enableExp && m_enableEcn && m_ecnInCycle && !m_lossInCycle && !m_isEce)
    {
      if (m_inflightHi != std::numeric_limits<int>::max ())
        {
          m_inflightHi++;
          uint64_t m_bwHiTmp = m_bwHi.GetBitRate () + (m_bwHi.GetBitRate () / m_rtProp.Get().GetSeconds ());
...
bool
TcpBbr2::IsTimeToCruise (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  bool isFullLength = (Simulator::Now () - m_cycleStamp) > m_rtProp;

  // Reduce inflight to leave headroom
  if (tcb->m_bytesInFlight.Get() > InflightWithHeadroom ())
    {
      return false;
    }
  
  return tcb->m_bytesInFlight.Get() <= InFlight (tcb, 1) || isFullLength;
}

bool
TcpBbr2::IsTimeToProbeRenoCoexistence ()
{
  NS_LOG_FUNCTION (this);
...
void
TcpBbr2::UpdateCyclePhase (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  ...
  case BbrBwPhase::BBR_BW_PROBE_UP:
    if (m_prevProbeTooHigh && tcb->m_bytesInFlight.Get() >= m_inflightHi)
      {
        m_isRiskyProbe = true;
      }
    else if (isFullLength && tcb->m_bytesInFlight.Get() >= InFlight (tcb, m_pacingGain))
      {
        isQueuing = true;
      }
...
void
TcpBbr2::EnterProbeDown (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  ResetCongestionSignals ();
...
void
TcpBbr2::EnterProbeUp (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::EnterProbeRefill (Ptr<TcpSocketState> tcb, uint32_t bwProbeUpRounds)
{
...
void
TcpBbr2::BoundCwndForInflightModel (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbr2::UpdateEcn (Ptr<TcpSocketState> tcb, const struct RateSample * rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  ...
  // Note: Simplified ECN handling due to missing m_delivered in current ns-3
  uint32_t delivered = rs->m_delivered;
  uint32_t deliveredEce = rs->m_deliveredEce;
  ...
  m_alphaLastDelivered = rs->m_delivered;
...
void
TcpBbr2::CheckEcnTooHighStartup (Ptr<TcpSocketState> tcb, const struct RateSample * rs, uint32_t ratio)
{
...
uint32_t
TcpBbr2::GetSsThresh (Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
