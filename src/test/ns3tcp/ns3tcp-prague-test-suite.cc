 ...
 * \ingroup tests
 *
 * \brief Application used in Ns3TcpPragueTestCase1
 */
class PragueSourceApplication : public Application 
{
public:

  PragueSourceApplication ();
  virtual ~PragueSourceApplication();
...
PragueSourceApplication::PragueSourceApplication ()
  : m_socket (0), 
    m_peer (), 
    m_packetSize (0), 
    m_nPackets (0), 
...
/* static */
TypeId
PragueSourceApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("PragueSourceApplication")
    .SetParent<Application> ()
    .SetGroupName ("Stats")
    .AddConstructor<PragueSourceApplication> ()
    ;
  return tid;
}
  
void
PragueSourceApplication::Setup (Ptr<Socket> socket, Address address, uint32_t packetSize, uint32_t nPackets, DataRate dataRate)
{
  m_socket = socket;
  m_peer = address;
...
void
PragueSourceApplication::StartApplication (void)
{
  m_running = true;
  m_packetsSent = 0;
  m_socket->Bind ();
...
void 
PragueSourceApplication::StopApplication (void)
{
  m_running = false;
...
void 
PragueSourceApplication::SendPacket (void)
{
...
void 
PragueSourceApplication::ScheduleTx (void)
{
  if (m_running)
    {
      Time tNext (Seconds (m_packetSize * 8 / static_cast<double> (m_dataRate.GetBitRate ())));
      m_sendEvent = Simulator::Schedule (tNext, &PragueSourceApplication::SendPacket, this);
    }
}
