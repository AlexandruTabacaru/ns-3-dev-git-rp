...
void
SourceApplication::Setup(Ptr<Socket> socket,
                         Address address,
                         uint32_t packetSize,
                         uint32_t nPackets,
                         DataRate dataRate)
...
Ns3TcpPragueTestCase1::Ns3TcpPragueTestCase1()
    : TestCase("Check to see that the ns-3 TCP congestion window works as expected for "
...
Ns3TcpPragueTestCase1::CwndChange(uint32_t oldCwnd, uint32_t newCwnd)
{
    ...
    if (newCwnd == 0)
    {
        std::cout << "Cwnd changed to zero" << std::endl;
    }
    NS_LOG_DEBUG("Cwnd change event " << m_responses.GetN() << " at " << Now().As(Time::S) << " "
                                      << oldCwnd << " " << newCwnd);
    uint32_t segmentSize = 1448;
    cwndStream << std::fixed << std::setprecision(6) << Simulator::Now().GetSeconds()
               << std::setw(12) << newCwnd / segmentSize << std::endl;
}
...
Ns3TcpPragueTestCase1::ScheduleFirstTcpCwndTraceConnection(void)
{
    Config::ConnectWithoutContext("/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow",
                                  MakeCallback(&Ns3TcpPragueTestCase1::CwndChange, this));
}
...
Ns3TcpPragueTestCase1::DoRun(void)
{
    ...
    NS_LOG_INFO("first TCP: " << firstTcpTypeId.GetName() << "; M3 queue: "
                              << m3QueueTypeId.GetName() << "; control: " << controlScenario);
    ...
    Simulator::Schedule(startTime + MicroSeconds(10),
                        &Ns3TcpPragueTestCase1::ScheduleFirstTcpCwndTraceConnection,
                        this);
    ...
    // //NS_TEST_ASSERT_MSG_EQ (m_responses.GetN (), N_EVENTS, "Unexpected number of cwnd change
    // events");
    ...
    // NS_TEST_ASSERT_MSG_EQ (event.m_newCwnd, (MSS * 15)/2, "Wrong new cwnd value in cwnd change
    // event " << 10);
    ...
    // NS_TEST_ASSERT_MSG_EQ (m_responses.Get (16).m_newCwnd, (MSS * 19)/2, "Wrong new cwnd value in
    // cwnd change event " << 16);

    // // partial ack again of 3 segments after one more acks, cwnd reset to 7.5
    // NS_TEST_ASSERT_MSG_EQ (m_responses.Get (18).m_newCwnd, (MSS * 15)/2, "Wrong new cwnd value in
    // cwnd change event " << 18);
    ...
    // NS_TEST_ASSERT_MSG_EQ (m_responses.Get (25).m_newCwnd,  2412, "Wrong new cwnd value in cwnd
    // change event " << 25);
    ...
    //     NS_TEST_ASSERT_MSG_EQ (m_responses.Get (i).m_newCwnd, cwnd, "Wrong new cwnd value in cwnd
    //     change event " << i);
    //   }
    // NS_LOG_DEBUG ("Reading out the cwnd event log");
    // for (uint32_t i = 0; i < N_EVENTS; ++i)
    // {
    //   NS_LOG_DEBUG ("i: " << i << " newCwnd: " << m_responses.Get(i).m_newCwnd << " newCwnd
    //   segments " << static_cast<double> (m_responses.Get(i).m_newCwnd)/MSS);
    // }
    Simulator::Destroy();
}

void
Ns3TcpPragueTestCase1::VerifyCwndRun(uint32_t beginIdx,
                                     uint32_t endIdx,
                                     ...
                                     uint32_t mss)
{
    ...
    for (uint32_t i = beginIdx, to = initialCwnd; i < endIdx; ++i, to += mss)
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(event.m_newCwnd,
                              to,
