...
TraceM1QueueLength(std::ofstream* ofStream, DataRate m1LinkRate, uint32_t oldVal, uint32_t newVal)
{
    ...
    *ofStream << Simulator::Now().GetSeconds() << " " << std::fixed
              << static_cast<double>(newVal * 8) / (m1LinkRate.GetBitRate() / 1000) << std::endl;
}
...
TraceM3QueueLength(std::ofstream* ofStream, DataRate m3LinkRate, uint32_t oldVal, uint32_t newVal)
{
    // output in units of ms
    *ofStream << Simulator::Now().GetSeconds() << " " << std::fixed
              << static_cast<double>(newVal * 8) / (m3LinkRate.GetBitRate() / 1000) << std::endl;
}
...
TraceFirstThroughput(std::ofstream* ofStream, Time throughputInterval)
{
    *ofStream << Simulator::Now().GetSeconds() << " "
              << g_firstBytesReceived * 8 / throughputInterval.GetSeconds() / 1e6 << std::endl;
...
{
    Config::ConnectWithoutContext("/NodeList/9/ApplicationList/*/$ns3::PacketSink/Rx",
...
{
    Config::ConnectWithoutContext("/NodeList/10/ApplicationList/*/$ns3::PacketSink/Rx",
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("firstTcpThroughputTraceFile",
                 "filename for throughput tracing",
                 firstTcpThroughputTraceFile);
    cmd.AddValue("secondTcpRttTraceFile", "filename for second rtt tracing", secondTcpRttTraceFile);
    cmd.AddValue("secondTcpCwndTraceFile",
                 "filename for second cwnd tracing",
                 secondTcpCwndTraceFile);
    cmd.AddValue("secondTcpThroughputTraceFile",
                 "filename for second throughput tracing",
                 secondTcpThroughputTraceFile);
    cmd.AddValue("m1DropTraceFile", "filename for m1 drops tracing", m1DropTraceFile);
    cmd.AddValue("m1DropsFrequencyTraceFile",
                 "filename for m1 drop frequency tracing",
                 m1DropsFrequencyTraceFile);
    ...
    cmd.AddValue("m3MarksFrequencyTraceFile",
                 "filename for m3 mark frequency tracing",
                 m3MarksFrequencyTraceFile);
    ...
    NS_LOG_DEBUG("first TCP: " << firstTcpTypeId.GetName()
                               << "; second TCP: " << secondTcpTypeId.GetName() << "; M3 queue: "
    ...
    qd->TraceConnectWithoutContext(
        "BytesInQueue",
        MakeBoundCallback(&TraceM1QueueLength, &m1LengthOfStream, link3Rate));
    ...
    qd->TraceConnectWithoutContext(
        "BytesInQueue",
        MakeBoundCallback(&TraceM3QueueLength, &m3LengthOfStream, link5Rate));
